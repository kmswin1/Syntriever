...
class ModifiedSoftNNLoss(nn.Module):
    ...
    def forward(self, sentence_features: Iterable[dict[str, Tensor]], labels: Tensor) -> Tensor:
        ...
        score1, score2, score3, score4 = scores.chunk(4, dim=1)
        score = score1 + score2 + score3
        score = torch.cat([score, score4], dim=-1)
...
triplets = []
qids = list(qrels) 
f_name = f"datasets/{dataset}_synthetic.jsonl"
...
with open(f_name, "r") as f:
    for line in f:
        ...
        if query not in queries.values():
            continue
        if line["negative_passage"] not in negs.keys():
            negs[line["q_id"]] = []
        cots[line["q_id"]] = line["cot"]
        if "yes" in line["positive_verification"].lower():
            pos[line["q_id"]] = line["positive_passage"]
        elif "no" in line["positive_verification"].lower():
            negs[line["q_id"]].append(line["positive_passage"])
        negs[line["q_id"]].append(line["negative_passage"])

#### Provide any sentence-transformers or HF model
if model_name == "contriever":
    model_path= "facebook/contriever""
elif model_name == "e5":
    model_path= f"intfloat/e5-base"
...
retriever = TrainRetriever(model=model, batch_size=60)

#### Prepare training samples
train_samples = retriever.load_train_hard_neg(corpus, queries, qrels, negs, pos, cots)
random.seed(42)
random.shuffle(train_samples)
train_dataloader = retriever.prepare_train_triplets(train_samples)

#### Training SBERT with cosine-product
train_loss = ModifiedSoftNNLoss(model=retriever.model)
...
model_save_path = os.path.join(pathlib.Path(__file__).parent.absolute(), "output", "{}-sft-{}".format(model_name, dataset))
