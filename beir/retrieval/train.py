...
class SyntrieverDataset(Dataset):
    """
...
class TrainRetriever:
    ...
    def load_train_hard_neg(self, corpus: Dict[str, Dict[str, str]], queries: Dict[str, str], 
                   qrels: Dict[str, Dict[str, int]], negs: Dict[str, str], pos: Dict[str, str], cots: Dict[str, str]) -> List[InputExample]:
        ...
        for idx, start_idx in enumerate(trange(0, len(query_ids), self.batch_size, desc='Adding Input Examples')):
            ...
            for query_id in query_ids_batch:
                for corpus_id, score in qrels[query_id].items():
                    if score >= 1: # if score = 0, we don't consider for training
                        try:
                            if query_id not in pos.keys():
                                s1 = queries[query_id]
                                s2 = corpus[corpus_id].get("title") + " " + corpus[corpus_id].get("text")
                                train_samples.append(InputExample(guid=idx, texts=[s1, s2, s2, cots[query_id], negs[query_id][0]]))
                                train_samples.append(InputExample(guid=idx, texts=[s1, s2, s2, cots[query_id], negs[query_id][1]]))
                            else:
                                s1 = queries[query_id]
                                s2 = corpus[corpus_id].get("title") + " " + corpus[corpus_id].get("text")
                                train_samples.append(InputExample(guid=idx, texts=[s1, s2, pos[query_id], cots[query_id], negs[query_id][0]]))
                        except KeyError:
                            logging.error("Error: Key {} not present in corpus!".format(corpus_id))
    ...
    def load_train_hard_neg_po(self, corpus: Dict[str, Dict[str, str]], queries: Dict[str, str], 
                   qrels: Dict[str, Dict[str, int]], negs: Dict[str, str], pos: Dict[str, str]) -> List[InputExample]:
            ...
            for query_id in query_ids_batch:
                for corpus_id, score in qrels[query_id].items():
                    if score >= 1: # if score = 0, we don't consider for training
                        try:
                            s1 = queries[query_id]
                            for p, n in zip(pos[s1], negs[s1]):
                                train_samples.append(InputExample(guid=idx, texts=[s1, p, n]))
                        except KeyError:
                            logging.error("Error: Key {} not present in corpus!".format(corpus_id))
